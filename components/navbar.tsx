  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="relative h-11 w-11 shrink-0 overflow-hidden rounded-full border-2 border-orange-500/40 bg-white shadow-sm ring-2 ring-blue-900/10 dark:bg-slate-900">
            <Image
              src="/assets/logo.png"
        ...
        <nav className="hidden items-center gap-1 lg:flex xl:gap-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className="rounded-lg px-3 py-1.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-orange-600 dark:text-slate-200 dark:hover:bg-slate-800/80 dark:hover:text-orange-400"
            >
              {link.name}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Button
              size="sm"
              className="gap-1.5 rounded-full bg-emerald-600 px-3.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-700"
            >
          ...
          <a href="tel:+8801780616187" className="hidden md:inline-flex">
            <Button
              variant="outline"
              size="sm"
              className="gap-1.5 rounded-full border-blue-900/30 text-xs font-semibold text-[#0F2C59] hover:bg-blue-50 dark:border-blue-400/40 dark:text-blue-300 dark:hover:bg-blue-950/50"
            >
          ...
          <Button
            ...
            className="h-9 w-9 text-slate-700 lg:hidden dark:text-slate-200"
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="border-b border-slate-200 bg-white/95 px-4 pt-3 pb-6 shadow-xl backdrop-blur-xl lg:hidden dark:border-slate-800 dark:bg-[#081026]/95">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/80 p-2.5 text-sm font-medium text-slate-800 transition-colors hover:border-orange-200 hover:bg-orange-50/50 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:bg-slate-800"
                >
          ...
          <div className="mt-4 flex flex-col gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            ...
            >
              <Button className="w-full gap-2 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700">
                ...
                সরাসরি হোয়াটসঅ্যাপ (০১৭৮০-৬১৬১৮৭)
              </Button>
            </a>
